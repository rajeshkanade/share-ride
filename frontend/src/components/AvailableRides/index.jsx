...
const AvailableRides = ({isShow , rideConfirmData , setBookRideInvisible}) => {
  ...
  console.log("is looking driver : ", isLookingDriver);
  useEffect(() => {
  ...
  return (
    ...
    <>
      <div className="col-span-1 relative h-full">
         ...
         {
          (isRideConfirm) ? <div className={`absolute bottom-0 left-0 w-full bg-white rounded-lg shadow-md p-4   transition-all duration-1000 overflow-hidden  ${
        (isRideConfirm)
          ? "h-full visible opacity-1"
          : `h-0 invisible opacity-0`
      }`}><DriverGetForRide vehicleData={vehicleData} rideConfirmData={rideConfirmData}/></div>
          :  ( (!isLookingDriver) ?
            <BookRidePanel isBookRide={isBookRide} vehicleData={vehicleData} setIsLookingDriver={(data)=>{setIsLookingDriver(true)}} createRide={createRide} isLookingDriver={isLookingDriver} setIsBookRide={setIsBookRide}/>
          : <LookingDriverPanel isLookingDriver={isLookingDriver} vehicleData={vehicleData} setIsLookingDriver={setIsLookingDriver} isBookRide={isBookRide} setIsBookRide={setIsBookRide}/>)
         }
          </div> 
    </>
  )
